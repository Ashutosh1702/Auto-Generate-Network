...
import { FiSmartphone, FiMessageCircle, FiCamera, FiClock, FiCheck, FiSend, FiPlus, FiAlertTriangle } from 'react-icons/fi'
...
const MobileAppSection = () => {
  ...
  const features = [
    { title: 'Job Updates', icon: <FiClock />, desc: 'Clock in/out of jobs in real-time straight from the workshop ramp.' },
    { title: 'Photo Uploads', icon: <FiCamera />, desc: 'Attach damage photos directly to job cards for visual diagnostics.' },
    { title: 'Customer Chat', icon: <FiMessageCircle />, desc: 'Instant SMS & Email chat to send updates and gain quick repair approvals.' },
    { title: 'Digital VHCs', icon: <FiSmartphone />, desc: 'Perform digital vehicle health check checklists and share reports instantly.' },
  ];
      ...
      <div className="bg-[#050816] rounded-2xl p-4 border border-white/5 mb-3 text-left">
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <div className="text-[10px] text-gray-500 font-bold mt-0.5">Ford Fiesta • Silver</div>
          </div>
        ...
        <div className="space-y-2.5 text-[11px] font-semibold">
          ...
          <div className="flex justify-between">
            ...
            <span className="text-cyan-400 font-black tracking-wider">{formatElapsed(elapsedSeconds)}</span>
          </div>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-white/5">
          ...
          <div className="text-[8.5px] text-gray-500 text-center mt-2 font-bold">65% Completed</div>
        </div>
      </div>
    ...
    <div className="flex-grow flex flex-col bg-[#111827] rounded-t-3xl pt-5 px-5 border-t border-white/5 relative h-full text-left">
      <div className="flex justify-between items-center mb-3">
        <div>
          ...
          <span className="text-[8px] text-gray-500 font-bold">AB12 CDE • Ford Fiesta</span>
        </div>
        <span className="bg-blue-500/10 text-cyan-400 text-[8px] font-black px-2 py-0.5 rounded border border-blue-500/20">
          2 UPLOADED
        </span>
      ...
      <div className="grid grid-cols-2 gap-2.5 mb-3">
        {[
          ...
          <div key={i} className="bg-[#050816] border border-white/5 rounded-xl p-2 flex flex-col justify-between aspect-[4/3] relative overflow-hidden group">
            ...
            <div className="flex justify-between items-start z-10">
              <span className="text-[7.5px] font-black text-white truncate max-w-[75%]">{img.name}</span>
            ...
            <span className="text-[6.5px] text-gray-500 font-bold z-10 mt-auto">{img.desc}</span>
          </div>
        ))}
        ...
        <div className="border border-dashed border-white/10 hover:border-blue-500/40 rounded-xl bg-white/5 flex flex-col items-center justify-center p-2 cursor-pointer transition-colors aspect-[4/3] select-none text-center">
          ...
          <span className="text-[7.5px] font-black text-gray-400">Add Diagnostics Image</span>
        </div>
      </div>
    ...
    <div className="flex-grow flex flex-col bg-[#111827] rounded-t-3xl pt-4 px-4 border-t border-white/5 relative h-full select-none justify-between">
      <div className="flex justify-between items-center border-b border-white/5 pb-2 shrink-0">
        <div className="flex items-center space-x-2">
          <div className="w-6 h-6 rounded-full bg-blue-500/20 text-blue-400 font-black flex items-center justify-center text-[9px] border border-blue-500/30">SJ</div>
          <span className="text-white font-bold text-[10.5px]">Sarah Jenkins</span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-2 py-3 text-left max-h-[170px] select-none">
        {[
          ...
          { sender: "mechanic", text: "Finished checks. Advisories loaded. Front brake pads need replacing." },
          { sender: "customer", text: "Please replace them. How much?" },
          { sender: "mechanic", text: "Front pads are £60.00 fitted. Added to invoice." }
        ].map((msg, i) => (
          <div key={i} className={`flex ${msg.sender === "customer" ? "justify-start" : "justify-end"}`}>
            <div className={`p-2 rounded-xl text-[7.5px] font-semibold leading-relaxed max-w-[80%] relative ${
              msg.sender === "customer" 
    ...
    <div className="flex-grow flex flex-col bg-[#111827] rounded-t-3xl pt-5 px-5 border-t border-white/5 relative h-full text-left justify-between">
      <div className="flex justify-between items-center border-b border-white/5 pb-2.5 shrink-0">
        <div>
          ...
          <span className="text-[7.5px] text-gray-500 font-bold">AB12 CDE • Ford Fiesta</span>
        </div>
      ...
      <div className="space-y-1.5 py-3 flex-grow justify-center flex flex-col">
        {[
          { name: "Engine & Oil Level", status: "HEALTHY", color: "text-green-400 bg-green-500/10 border-green-500/20", dot: "🟢" },
          { name: "Tyres & Tracking", status: "ADVISORY", color: "text-amber-400 bg-amber-500/10 border-amber-500/20", dot: "🟡" },
          { name: "Brakes & Discs", status: "CRITICAL", color: "text-red-400 bg-red-500/10 border-red-500/20", dot: "🔴" },
          { name: "Lights & Electrical", status: "HEALTHY", color: "text-green-400 bg-green-500/10 border-green-500/20", dot: "🟢" }
        ].map((item, i) => (
          <div key={i} className="flex justify-between items-center bg-[#050816] border border-white/5 rounded-xl p-2 text-[8px] font-bold">
            <div className="flex items-center space-x-1.5">
              <span>{item.dot}</span>
              <span className="text-gray-300 font-semibold">{item.name}</span>
            </div>
            <span className={`text-[7px] font-black px-1.5 py-0.5 rounded border shrink-0 ${item.color}`}>
              {item.status}
            </span>
          </div>
        ))}
          ...
          <div className="order-2 md:order-1 space-y-8 text-left z-10">
            ...
            >
              <h2 className="text-3xl md:text-5xl font-bold text-white leading-tight mb-6">
                Manage Your Garage From <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400">Anywhere</span>
              </h2>
              ...
              {features.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`mb-3 transition-colors ${isActive ? "text-cyan-400" : "text-gray-500"}`}>
                      {React.cloneElement(item.icon, { className: 'w-6 h-6' })}
                    </div>
                    <div>
                      <h4 className={`font-bold text-sm mb-1.5 transition-all ${isActive ? "text-blue-400" : "text-white"}`}>
                        {item.title}
                      </h4>
            ...
            >
              <button className="flex items-center gap-3 bg-white/5 hover:bg-white/10 border border-white/10 px-6 py-3.5 rounded-xl text-white transition-colors group cursor-pointer">
                ...
                <div className="text-left">
                  <div className="text-[10px] text-gray-400 uppercase tracking-wide font-bold">Download on the</div>
                  <div className="font-extrabold leading-none mt-0.5 text-xs">App Store</div>
                </div>
              </button>
              <button className="flex items-center gap-3 bg-white/5 hover:bg-white/10 border border-white/10 px-6 py-3.5 rounded-xl text-white transition-colors group cursor-pointer">
                <FaGooglePlay className="w-5 h-5 group-hover:text-cyan-400 text-gray-400 transition-colors" />
                <div className="text-left">
                  <div className="text-[10px] text-gray-400 uppercase tracking-wide font-bold">GET IT ON</div>
                  <div className="font-extrabold leading-none mt-0.5 text-xs">Google Play</div>
                </div>
              </button>
            </motion.div>
          </div>
              ...
              <div className="flex-1 overflow-hidden flex flex-col bg-[#050816]">
                <div className="bg-gradient-to-b from-blue-600/10 to-transparent p-5 pb-2 select-none shrink-0">
                  <div className="flex justify-between items-center mb-5 text-left">
                    <div>
                      <div className="text-[8.5px] text-cyan-400 font-black tracking-widest uppercase mb-0.5">Mechanic View</div>
                      <div className="text-base font-extrabold text-white">John Smith</div>
                    </div>
                    <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-blue-500 to-cyan-400 p-0.5">
                      <div className="w-full h-full bg-[#111827] rounded-full flex items-center justify-center text-white font-extrabold text-xs">JS</div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 mb-3">
                    {[
                      ...
                      { icon: <FiMessageCircle />, label: 'Message', tabIdx: 2 },
                    ].map((btn, i) => {
                      ...
                      return (
                        ...
                        >
                          {React.cloneElement(btn.icon, { className: 'w-4.5 h-4.5 mb-1.5' })}
                          <span className="text-[8.5px] font-black">{btn.label}</span>
                        </div>
                      );
                    })}
                  </div>
