        ...
        <div className="grid md:grid-cols-3 gap-8 items-stretch">
          {plans.map((plan, idx) => (
            <motion.div
              ...
              whileHover={{ y: -10 }}
              className={`relative bg-[#111827] rounded-3xl p-8 flex flex-col transition-all duration-300 ${
                plan.isPopular 
                  ? 'border-2 border-indigo-500 shadow-[0_0_40px_rgba(79,70,229,0.2)]' 
                  : 'border border-white/5 hover:border-white/20 shadow-lg'
              }`}
            >
              {plan.isPopular && (
                <div className="absolute top-0 inset-x-0 -translate-y-1/2 flex justify-center">
                  <div className="bg-gradient-to-r from-indigo-500 to-purple-500 text-white text-xs font-bold px-4 py-1.5 rounded-full uppercase tracking-wider shadow-lg">
                    Most Popular
                  </div>
                </div>
              )}
