      ...
      icon: (
        <div className="relative w-full h-16 bg-slate-950/80 rounded-lg border border-white/5 flex items-center justify-center p-2">
          <div className="grid grid-cols-3 gap-1 w-full h-full">
            {[...Array(6)].map((_, i) => (
              <div key={i} className={`rounded-[3px] border border-white/5 ${i === 2 ? 'bg-indigo-500/20' : 'bg-white/5'}`} />
            ))}
          </div>
        </div>
      ),
        ...
        <div className="relative w-full h-16 bg-slate-950/80 rounded-lg border border-white/5 flex flex-col justify-end p-2 space-y-1.5">
          <div className="flex justify-between items-end h-8 px-2">
            {[20, 35, 55, 45, 75].map((h, i) => (
              <div key={i} style={{ height: `${h}%` }} className={`w-1.5 rounded-t-sm ${i === 4 ? 'bg-pink-500' : 'bg-white/20'}`} />
            ))}
          </div>
        </div>
      ),
      ...
      icon: (
        <div className="relative w-full h-16 bg-slate-950/80 rounded-lg border border-white/5 flex items-center justify-center p-2">
          <div className="flex space-x-1.5">
            <div className="w-5 h-5 rounded-full bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center text-[10px]">👤</div>
            <div className="w-5 h-5 rounded-full bg-teal-500/10 border border-teal-500/20 flex items-center justify-center text-[10px]">👤</div>
          </div>
        </div>
      ),
    },
  ...
  return (
    <section className="py-20 px-6 md:px-12 max-w-7xl mx-auto space-y-12">
      <div className="text-center max-w-xl mx-auto space-y-3">
        <h2 className="text-3xl font-extrabold text-white tracking-tight">Website & Support Solutions</h2>
        <p className="text-gray-400 text-sm md:text-base font-medium">
          Professional platforms designed specifically to grow your garage operation
        </p>
      </div>
