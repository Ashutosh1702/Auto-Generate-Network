...
const getFeatureLogoOrIcon = (name, idx) => {
  ...
  if (logoSrc) {
    return (
      <div className="bg-gradient-to-b from-white to-[#f1f5f9] px-4 py-2.5 rounded-2xl border border-slate-200/80 shadow-[0_4px_10px_rgba(0,0,0,0.06),inset_0_-2px_0_rgba(15,23,42,0.08),inset_0_1px_0_rgba(255,255,255,1)] flex items-center justify-center h-14 w-fit min-w-[95px] transform hover:scale-[1.05] transition-transform duration-300">
        <img src={logoSrc} alt={name} className="h-8 object-contain max-w-[125px] select-none" />
      </div>
    );
  }
  ...
  return (
    <div className="w-12 h-12 rounded-2xl bg-gradient-to-b from-blue-500/20 to-blue-600/5 border border-blue-500/35 text-blue-400 flex items-center justify-center shadow-[0_4px_10px_rgba(59,130,246,0.15),inset_0_1px_0_rgba(255,255,255,0.1)] select-none shrink-0 transform hover:scale-[1.05] transition-transform duration-300">
      {IconComponent ? <IconComponent className="w-5 h-5" /> : <span className="font-extrabold text-sm">{idx + 1}</span>}
    </div>
  );
};
...
const GarageManagementSystem = () => {
  ...
  const keyFeatures = [
    { name: "TecRMI Inside", desc: "Access manufacturer service/repair times and detailed operations configuration." },
    { name: "Automated Reminders", desc: "Built-in scheduler triggers automated SMS & email campaigns for vehicle MOT dates." },
    { name: "Courtesy Vehicles", desc: "Manage agreement templates and check-outs for courtesy cars in real-time." },
    { name: "Calendar Scheduling", desc: "Smart drag-and-drop bay calendar configuration to coordinate ramp loads." },
    { name: "Customer Management", desc: "Central database CRM to manage customer vehicle diagnostics histories and invoicing." },
    { name: "Customer Portal", desc: "Transparent client portal allowing drivers to review and accept estimates online." },
    { name: "Autodata Integration", desc: "OEM technical repair specifications and interactive wiring diagrams." },
    { name: "GSF Parts Integration", desc: "Direct catalog parts ordering link to GSF distributors at the click of a button." },
    { name: "Partslink24 Integration", desc: "Live wholesale inventory connections for direct spare parts procurement." },
    { name: "Postcode Lookup", desc: "Direct address verification fields to register vehicles and clients swiftly." },
    { name: "Tyre Suppliers Stock APIs", desc: "Compare prices and access live stock connections from leading UK tyre wholesalers." },
    { name: "Custom Job Cards", desc: "Auto-generated smart job cards outlining exact vehicle specs and required checklists." },
    { name: "Invoicing & Estimates", desc: "Streamline billing with professional estimates, automated jobsheets, and instant invoices." },
    { name: "Xero Integration", desc: "One-click financial accounting synchronization to prevent double data entries." },
    { name: "Quickbooks Integration", desc: "Seamless accounting journal log updates for daily workshop revenue." },
    { name: "Sage Integration", desc: "Secure invoice transaction transfer to your Sage bookkeeping dashboards." },
    { name: "Stock Control", desc: "Automated parts inventory tracker linked to upcoming diagnostics schedules." },
    { name: "SMS / Email Communication", desc: "Unified messages inbox to communicate with clients about active repairs." },
    { name: "Web Bookings Gateway", desc: "Enable customers to book MOT slots and tyre fittings from your site 24/7." },
    { name: "Technician Mobile App", desc: "Mobile check-off list and visual diagnostic upload apps for mechanics on Android/iOS." },
    { name: "Administrator Mobile App", desc: "Track revenue logs, check scheduling loads, and view active tickets on the go." },
    { name: "Customer Booking App", desc: "Custom branded iOS & Android apps under your own garage name to secure bookings." },
    { name: "Vehicle Sales", desc: "Manage pre-owned vehicle logs, sales listings, and historic vehicle invoices." },
    { name: "Inspection Form Manager", desc: "Generate electronic vehicle inspection reports with photo attachments." },
    { name: "Bookkeeping & Reports", desc: "Generate advanced analytical graphs tracking margins, labor rates, and profitability." },
    { name: "Data Migration Support", desc: "Assistance migrating CRM and inventory historical files into the new GMS suite." },
    { name: "Multi-Site Control", desc: "Manage scheduling metrics, staff ritas, and parts transfers across branches." },
    { name: "Automatic Updates", desc: "Seamless cloud updates ensuring your workshop always runs the latest software tools." },
    { name: "DVLA Database Sync", desc: "Instant MOT history, VIN, engine code, and tyre sizing lookups by registration." },
    { name: "Ramp Configuration", desc: "Configure custom bay loads, load limits, and technician lift schedules." },
    { name: "Stapletons Tyre Integration", desc: "Browse wholesale stock from Stapletons Tyre directly in the parts portal." },
    { name: "BMTR Tyre API", desc: "Query live tyre stock and delivery time estimates from BMTR wholesale portal." },
    { name: "Group Tyre Stock", desc: "Connect directly to Group tyres supplier database for active tyre bookings." },
    { name: "Oak Tyre Integration", desc: "Live integration with Oak Tyres for tyres catalogue lookups and prices." },
    { name: "Bond Integration", desc: "Seamless integration with Bond International for live tyre stock and pricing." },
    { name: "Secure Cloud Backups", desc: "Automatic daily data replication keeping your database secure and encrypted." },
    { name: "Personnel Rota Logs", desc: "Track labor hours, technician job efficiency, and configure mechanic commissions." }
  ];
  ...
  const pricingFeatures = [
    ...
    { name: "SMS/MOT Reminders (Fair Usage)", p1: "300 FOC PM", p2: "400 FOC PM", p3: "Unlimited*" },
    { name: "VRMs (Fair Usage)", p1: "300 PM", p2: "Unlimited*", p3: "Unlimited*" },
    { name: "Advanced VRM info", p1: "Yes", p2: "Yes", p3: "Yes" },
    { name: "Postcode Lookup", p1: "200pm", p2: "300pm", p3: "Unlimited*" },
    { name: "Tyre Suppliers Stock APIs", p1: "1 Supplier", p2: "Unlimited*", p3: "Unlimited*" },
    { name: "Multi users", p1: "3 Users", p2: "Unlimited*", p3: "Unlimited*" },
    { name: "Website Solutions", p1: "Not Included", p2: "Not Included", p3: "Free Unlimited" },
    { name: "Manufacturers Service/Repair Times", p1: "Optional £", p2: "Yes", p3: "Yes" },
    { name: "Stock System", p1: "Optional £", p2: "Yes", p3: "Yes" },
    { name: "Accounting Package Integration", p1: "Optional £", p2: "Yes", p3: "Yes" },
    { name: "Service Visuals", p1: "Optional £", p2: "Yes", p3: "Yes" },
    { name: "Vehicle Sales", p1: "Optional £", p2: "Optional £", p3: "Yes" },
    { name: "Full Technical Data (Autodata)", p1: "Optional £", p2: "Optional £", p3: "Yes" },
    { name: "Courtesy Cars", p1: "Optional £", p2: "Optional £", p3: "Yes" },
    { name: "Electronic Vehicle Inspections", p1: "Optional £", p2: "Optional £", p3: "Yes" },
    { name: "Technicians Mobile App", p1: "Optional £", p2: "Optional £", p3: "Yes" },
    { name: "Technicians Efficiency Reports", p1: "Optional £", p2: "Optional £", p3: "Yes" },
    { name: "Advanced Analytical Reports", p1: "Optional £", p2: "Optional £", p3: "Yes" },
    { name: "Set up cost", p1: "£500", p2: "£500", p3: "£500" },
    { name: "Data Migration", p1: "Optional £", p2: "Optional £", p3: "Optional £" },
          ...
          <div className="max-w-7xl mx-auto grid lg:grid-cols-12 gap-12 lg:gap-16 items-start relative z-10">
            ...
            <div className="lg:col-span-7 space-y-8 text-left">
              ...
              <p className="text-gray-400 text-base md:text-lg leading-relaxed max-w-2xl font-semibold">
                Effortlessly streamline your workshop operations, maximize booking loads, and auto-sync accounting with our comprehensive GMS.
              </p>
              ...
              <div className="relative group">
                ...
                <div className="relative bg-[#0c1222] border border-white/10 rounded-[30px] p-6 md:p-8 shadow-2xl backdrop-blur-md overflow-hidden">
                  ...
                  <p className="text-xs text-gray-400 text-center mb-6 select-none font-semibold">
                    Fill in details below and our UK team will contact you shortly.
                  </p>
                  ...
                  <AnimatePresence mode="wait">
                    {formSubmitted ? (
                      ...
                      >
                        ...
                        <h4 className="text-white font-bold text-lg">Inquiry Submitted!</h4>
                        <p className="text-xs text-gray-400 max-w-xs mx-auto leading-relaxed">
                          Thank you for your interest. An Auto Garage Network specialist will reach out to you within 24 hours.
                        </p>
                        <button
                          type="button"
                      ...
                      <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="relative flex items-center group/field">
                            ...
                            >
                              <option value="" disabled className="dark:bg-slate-900 dark:text-white bg-white text-slate-800">Please Select</option>
                              <option value="Elite Workshop" className="dark:bg-slate-900 dark:text-white bg-white text-slate-800">Elite Workshop (£135+VAT /mo)</option>
                              <option value="Elite ProMax" className="dark:bg-slate-900 dark:text-white bg-white text-slate-800">Elite ProMax Workshop (£235+VAT /mo)</option>
                              <option value="Elite ProMax Plus" className="dark:bg-slate-900 dark:text-white bg-white text-slate-800">Elite ProMax Plus Workshop (£375+VAT /mo)</option>
                              <option value="General Query" className="dark:bg-slate-900 dark:text-white bg-white text-slate-800">General Inquiry</option>
                            </select>
          ...
          <div className="max-w-7xl mx-auto space-y-12">
            <div className="text-center space-y-4 max-w-3xl mx-auto">
              ...
              <p className="text-gray-400 text-sm md:text-base leading-relaxed">
                Compare direct GMS features and choose the tier that fits your workshop's daily goals.
              </p>
            </div>
              ...
              <table className="w-full border-collapse text-left text-xs">
                <thead>
                  <tr className="border-b border-white/10 bg-[#050816]/80 text-gray-400 select-none">
                    <th className="p-5 font-black uppercase text-[10px] tracking-wider">Features Suite</th>
                    <th className="p-5 font-black uppercase text-[10px] tracking-wider text-center bg-white/5">
                      ...
                      <div className="text-emerald-400 text-lg font-black mt-1">£135<span className="text-[10px] font-normal text-gray-400">/mo + VAT</span></div>
                      <button
                        type="button"
                    ...
                    <th className="p-5 font-black uppercase text-[10px] tracking-wider text-center bg-indigo-500/10 border-x border-white/10">
                      <div className="text-indigo-300 text-sm">Elite ProMax</div>
                      <div className="text-indigo-400 text-lg font-black mt-1">£235<span className="text-[10px] font-normal text-gray-400">/mo + VAT</span></div>
                      <button
                        type="button"
                    ...
                    <th className="p-5 font-black uppercase text-[10px] tracking-wider text-center bg-white/5">
                      <div className="text-white text-sm">Elite ProMax Plus</div>
                      <div className="text-emerald-400 text-lg font-black mt-1">£375<span className="text-[10px] font-normal text-gray-400">/mo + VAT</span></div>
                      <button
                        type="button"
                ...
                <tbody>
                  {pricingFeatures.map((row, idx) => (
                    ...
                    >
                      <td className="p-4 font-bold text-gray-200 border-r border-white/5">{row.name}</td>
                      <td className="p-4 text-center border-r border-white/5 bg-white/[0.01] text-gray-200">
                        {row.p1 === "Yes" ? <FiCheck className="text-blue-400 w-4.5 h-4.5 mx-auto" /> : row.p1 === "No" ? <FiX className="text-red-400 w-4.5 h-4.5 mx-auto" /> : row.p1}
                      </td>
                      <td className="p-4 text-center bg-indigo-500/[0.02] border-r border-white/10 font-bold text-white">
                        {row.p2 === "Yes" ? <FiCheck className="text-blue-400 w-4.5 h-4.5 mx-auto" /> : row.p2 === "No" ? <FiX className="text-red-400 w-4.5 h-4.5 mx-auto" /> : row.p2}
                      </td>
                      <td className="p-4 text-center bg-white/[0.01] text-gray-200">
                        {row.p3 === "Yes" ? <FiCheck className="text-blue-400 w-4.5 h-4.5 mx-auto" /> : row.p3 === "No" ? <FiX className="text-red-400 w-4.5 h-4.5 mx-auto" /> : row.p3}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="grid md:hidden gap-6">
              {[
                ...
                <div key={i} className="bg-[#0c1222]/90 border border-white/10 rounded-[22px] p-6 space-y-4 shadow-xl">
                  <div className="flex justify-between items-center pb-3 border-b border-white/10">
                    <h3 className="font-extrabold text-white text-base">{plan.name}</h3>
                    <span className="text-indigo-400 font-black text-lg">{plan.price}/mo</span>
                  </div>
                  <ul className="space-y-3.5 text-xs text-gray-400">
                    {pricingFeatures.slice(0, 10).map((row, idx) => (
                      <li key={idx} className="flex justify-between">
                        <span className="font-medium">{row.name}</span>
                        <span className="font-bold text-white">{row[plan.listKey]}</span>
                      </li>
                    ))}
            ...
            >
              ...
              <div className="relative z-10 space-y-6 text-left">
                <div className="flex items-center gap-2 text-indigo-400 font-bold uppercase text-xs tracking-wider">
                  <FiAlertTriangle className="w-5 h-5 shrink-0" /> Fair Usage Policy (FUP) for SMS & VRMs
                </div>
                ...
                <div className="space-y-4 text-xs md:text-sm text-gray-400 leading-relaxed font-medium">
                  <p>
                    <strong>1. Allocation:</strong> Subscribed packages include specific allocations of SMS messages and Vehicle Registration Mark (VRM) lookups per billing cycle (monthly). Unused allocations do not roll over.
                  </p>
                  <p>
                    <strong>2. Fair Usage Limits:</strong> Customers must use services for standard business operations. Standard rates apply for excess usage beyond your limit (SMS: 10p per message, VRM: 10p per lookup).
                  </p>
                  <p>
                    <strong>3. Prohibited Use:</strong> Reselling lookup queries or utilizing automated scrapers is strictly prohibited under Mr. Bassi's security system.
                  </p>
                  <p>
                    <strong>4. Support & Monitoring:</strong> Notifications will be dispatched when approaching 90% usage limits. Contact support on <strong>01702 655556</strong> or <strong>support@autogaragenetwork.com</strong> for assistance.
                  </p>
                </div>
              </div>
            </motion.div>
        ...
        <section className="py-20 px-6 md:px-12 bg-[#0a0f24]/20 border-t border-white/5 relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto space-y-16">
            <div className="text-center space-y-4 max-w-3xl mx-auto">
              ...
              <p className="text-gray-400 text-sm md:text-base leading-relaxed">
                Everything you need to successfully run, automate, and expand your independent garage workshop.
              </p>
            </div>
 
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {keyFeatures
                .filter((item) => featureLogos[item.name] || item.name === "Xero Integration" || item.name === "Sage Integration")
                .map((item, idx) => (
                  <motion.div
                    key={idx}
                    whileHover={{ 
                  ...
                  >
                    ...
                    <h4 className="font-extrabold text-white text-sm mb-2 transition-colors duration-300 group-hover:text-indigo-300 relative z-10">{item.name}</h4>
                    <p className="text-[11px] text-gray-400 leading-relaxed font-medium flex-grow relative z-10">{item.desc}</p>
                  </motion.div>
                ))}
            </div>
          </div>
